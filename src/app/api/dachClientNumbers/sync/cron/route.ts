...
import { getDachClientNumbersAutoImportState, runCommitImport } from '../shared';
...
async function handleCronImport(request: Request) {
  ...
  const autoImportState = await getDachClientNumbersAutoImportState();
  ...
  if (!autoImportState.enabled) {
    ...
    if (supabase) {
      try {
        await supabase.from('dach_client_numbers_import_runs').insert({
          triggered_by: 'cron',
          status: 'skipped',
