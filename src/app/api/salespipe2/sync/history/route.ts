...
export async function GET(request: Request) {
  ...
  if (runId) {
    const { data: items, error } = await supabase
      .from('salespipe2_import_run_items')
      .select('*')
      .eq('run_id', runId)
    ...
    if (error) {
      if (error.message.includes('salespipe2_import_run_items') || error.message.includes('schema cache')) {
        return NextResponse.json(
          {
            success: false,
            error:
              'Import-Historie ist noch nicht initialisiert. Bitte zuerst supabase-salespipe2-import.sql in Supabase ausfuehren.',
          },
          { status: 500 }
        );
      }
  ...
  if (error) {
    if (error.message.includes('salespipe2_import_runs') || error.message.includes('schema cache')) {
      return NextResponse.json(
        {
          success: false,
          error:
