...
import { getPayStripeTerminalInstallationAutoImportState } from '../shared';
...
export async function GET() {
  const state = await getPayStripeTerminalInstallationAutoImportState();
  if (!state.success) {
...
export async function PUT(request: Request) {
  ...
  const { data, error } = await supabase
    ...
    .upsert(
      {
        key: PAY_STRIPE_TERMINAL_INSTALLATION_AUTO_IMPORT_KEY,
