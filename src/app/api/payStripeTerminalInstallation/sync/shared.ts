...
type ExtractResult =
  | {
      ...
      validRows: ParsedInstallationRow[];
      invalidRows: InvalidInstallationRow[];
...
function getDriveFolderId(): string | null {
  return process.env.GOOGLE_DRIVE_PAY_STRIPE_TERMINAL_INSTALLATION_FOLDER_ID?.trim() || null;
}
...
async function listZipFiles(): Promise<{ success: true; files: DriveFileMeta[] } | { success: false; status: number; error: string }> {
  ...
  if (!folderId) {
    return { success: false, status: 500, error: 'GOOGLE_DRIVE_PAY_STRIPE_TERMINAL_INSTALLATION_FOLDER_ID fehlt.' };
  }
...
async function getLatestUnprocessedZip(supabase: ReturnType<typeof createClient>, force = false) {
  ...
  const { data, error } = await supabase
    .from('pay_stripe_terminal_installation_source_files')
...
async function persistImportRun(params: {
  ...
  const { data: run, error: runError } = await supabase
    .from('pay_stripe_terminal_installation_import_runs')
    .insert({
  ...
  if (runError || !run?.id) {
    console.error('Pay Stripe Terminal Installation Import-Run Logging fehlgeschlagen:', runError?.message || 'Keine run.id');
    return;
  }
  ...
  if (allItems.length > 0) {
    await supabase.from('pay_stripe_terminal_installation_import_run_items').insert(allItems);
  }
}
...
export async function getPayStripeTerminalInstallationAutoImportState() {
  ...
  const { data, error } = await supabase
    ...
    .eq('key', PAY_STRIPE_TERMINAL_INSTALLATION_AUTO_IMPORT_KEY)
    .maybeSingle();
...
export async function extractPayStripeTerminalInstallationRows(options?: { force?: boolean }): Promise<ExtractResult> {
  ...
  if (csvEntries.length > 1) {
    warnings.push({
      rowNumber: 0,
      warning: `ZIP enthaelt ${csvEntries.length} CSV-Dateien. Verwendet wurde: ${csvEntry.entryName}`,
    });
  }

  const validRows: ParsedInstallationRow[] = [];
  const invalidRows: InvalidInstallationRow[] = [];
...
export async function runCommitImport(context?: { triggeredBy?: ImportTrigger; autoImportEnabled?: boolean; force?: boolean }) {
  ...
  const extracted = await extractPayStripeTerminalInstallationRows({ force: Boolean(context?.force) });
  ...
  try {
    const rowsForUpsert = extracted.validRows.map((row) => ({
      ...
      modified_at: sourceFile.modifiedTime,
      csv_entry_name: extracted.csvEntryName,
      payload: row.payload,
    ...
    const { data: existingRows, error: existingError } = rowNumbers.length
      ? await supabase
          .from('pay_stripe_terminal_installation_events')
    ...
    return {
      success: false,
      status: 500,
      error: error?.message || 'Unbekannter Fehler beim Pay Stripe Terminal Installation Import.',
      sourceFile,
    };
  }
}
...
export async function runDryRun(options?: { force?: boolean }) {
  const extracted = await extractPayStripeTerminalInstallationRows({ force: Boolean(options?.force) });
  if (!extracted.success) {
