...
import { getPayStripeTerminalInstallationAutoImportState, runCommitImport } from '../shared';
...
async function handleCronImport(request: Request) {
  ...
  const autoImportState = await getPayStripeTerminalInstallationAutoImportState();
  ...
  if (!autoImportState.enabled) {
    ...
    if (supabaseUrl && serviceRoleKey) {
      try {
        ...
        await supabase.from('pay_stripe_terminal_installation_import_runs').insert({
          triggered_by: 'cron',
          status: 'skipped',
