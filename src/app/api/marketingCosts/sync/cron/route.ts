...
import { getMarketingCostsAutoImportState, runCommitImport } from '../shared';
...
async function handleCronImport(request: Request) {
  ...
  const autoImportState = await getMarketingCostsAutoImportState();
  ...
  if (!autoImportState.enabled) {
    ...
    if (supabase) {
      try {
        await supabase.from('marketing_costs_import_runs').insert({
          triggered_by: 'cron',
          status: 'skipped',
