...
const MARKETING_COSTS_AUTO_IMPORT_KEY = 'marketing_costs_auto_import_enabled';
const DRIVE_SCOPE = ['https://www.googleapis.com/auth/drive.readonly'];
const MARKETING_COSTS_FALLBACK_FOLDER_ID = '18vqkJDffpi0X6ad-BTzRECEEVJ1c1n9s';
const MARKETING_COSTS_ZIP_NAME = 'marketing cost';
...
type ExtractResult =
  | {
      ...
      validRows: ParsedMarketingCostsRow[];
      invalidRows: InvalidMarketingCostsRow[];
...
function getDriveFolderId(): string {
  return (
    extractGoogleDriveFolderId(process.env.GOOGLE_DRIVE_MARKETING_COSTS_FOLDER_ID) ||
    extractGoogleDriveFolderId(process.env.GOOGLE_DRIVE_MARKETING_COSTS_FOLDER_URL) ||
    MARKETING_COSTS_FALLBACK_FOLDER_ID
  );
}
...
async function listZipFiles(): Promise<{ success: true; files: DriveFileMeta[] } | { success: false; status: number; error: string }> {
  ...
  const files = (response.data.files || [])
    ...
    .filter((f) => f.name.toLowerCase().includes(MARKETING_COSTS_ZIP_NAME));
...
async function getLatestUnprocessedZip(supabase: any, force = false) {
  ...
  if (listResult.files.length === 0) {
    return { success: false as const, status: 404, error: 'Keine ZIP-Datei "Marketing Cost" im Drive-Ordner gefunden.' };
  }
  ...
  const { data, error } = await supabase
    .from('marketing_costs_source_files')
  ...
  if (!pending) {
    return {
      ...
      status: 200,
      error: 'Keine neuen Marketing-Cost-ZIP-Dateien zu verarbeiten (alle bereits importiert).',
    };
  }
...
function scoreCsvEntryName(entryName: string): number {
  ...
  let score = 0;
  if (name.includes('marketing')) score += 200;
  if (name.includes('cost')) score += 200;
  if (name.includes('spend')) score += 100;
  if (name.includes('budget')) score += 75;
  return score;
}
...
async function persistImportRun(params: {
  ...
  const { data: run, error: runError } = await supabase
    .from('marketing_costs_import_runs')
    .insert({
  ...
  if (runError || !run?.id) {
    console.error('Marketing Cost Import-Run Logging fehlgeschlagen:', runError?.message || 'Keine run.id');
    return;
  }
  ...
  if (allItems.length > 0) {
    await supabase.from('marketing_costs_import_run_items').insert(allItems);
  }
}

export async function getMarketingCostsAutoImportState() {
  ...
  const { data, error } = await supabase
    ...
    .eq('key', MARKETING_COSTS_AUTO_IMPORT_KEY)
    .maybeSingle();
...
export async function extractMarketingCostsRows(options?: { force?: boolean }): Promise<ExtractResult> {
  const supabase = await getServerSupabase();
  if (!supabase) {
  ...
  const validRows: ParsedMarketingCostsRow[] = [];
  const invalidRows: InvalidMarketingCostsRow[] = [];
...
export async function runCommitImport(context?: { triggeredBy?: ImportTrigger; autoImportEnabled?: boolean; force?: boolean }) {
  ...
  const extracted = await extractMarketingCostsRows({ force: Boolean(context?.force) });
  ...
  try {
    ...
    return {
      success: false,
      status: 500,
      error: error?.message || 'Unbekannter Fehler beim Marketing Cost Import.',
      sourceFile,
    };
  }
}
...
export async function runDryRun(options?: { force?: boolean }) {
  const extracted = await extractMarketingCostsRows({ force: Boolean(options?.force) });
  if (!extracted.success) {
