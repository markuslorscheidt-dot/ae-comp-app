...
import { getChurnDriveAutoImportState, runCommitImport } from '../shared';
...
async function handleCronImport(request: Request) {
  ...
  const autoImportState = await getChurnDriveAutoImportState();
  ...
  if (!autoImportState.enabled) {
    ...
    if (supabaseUrl && serviceRoleKey) {
      try {
        ...
        await supabase.from('churn_drive_import_runs').insert({
          triggered_by: 'cron',
          status: 'skipped',
