...
import { getPhorestPayRevenueAutoImportState, runCommitImport } from '../shared';
...
async function handleCronImport(request: Request) {
  ...
  const autoImportState = await getPhorestPayRevenueAutoImportState();
  ...
  if (!autoImportState.enabled) {
    ...
    if (supabase) {
      try {
        await supabase.from('phorest_pay_revenue_import_runs').insert({
          triggered_by: 'cron',
          status: 'skipped',
