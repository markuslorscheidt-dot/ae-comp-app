...
export async function GET(request: Request) {
  ...
  if (runId) {
    ...
    if (error) {
      if (error.message.includes('leads_import_run_items') || error.message.includes('schema cache')) {
        return NextResponse.json(
          {
            success: false,
            error:
              'Import-Historie ist noch nicht initialisiert. Bitte zuerst supabase-leads-import.sql in Supabase ausfuehren.',
          },
          { status: 500 }
        );
      }
  ...
  if (error) {
    if (error.message.includes('leads_import_runs') || error.message.includes('schema cache')) {
      return NextResponse.json(
        {
          success: false,
          error:
