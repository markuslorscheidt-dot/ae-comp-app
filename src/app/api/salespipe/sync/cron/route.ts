...
import { getSalespipeAutoImportState, runCommitImport } from '../shared';
...
async function handleCronImport(request: Request) {
  ...
  const autoImportState = await getSalespipeAutoImportState();
  ...
  if (!autoImportState.enabled) {
    ...
    if (supabaseUrl && serviceRoleKey) {
      try {
        ...
        await supabase.from('salespipe_import_runs').insert({
          triggered_by: 'cron',
          status: 'skipped',
