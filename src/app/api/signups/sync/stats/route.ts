...
export async function GET() {
  ...
  const { count, error } = await supabase.from('signups_events').select('*', { count: 'exact', head: true });
  if (error) {
    return NextResponse.json(
      { success: false, error: `Sign-ups Statistik konnte nicht geladen werden: ${error.message}` },
      { status: 500 }
    );
  }
