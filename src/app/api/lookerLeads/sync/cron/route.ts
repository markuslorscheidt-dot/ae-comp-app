...
import { getLookerLeadsAutoImportState, runCommitImport } from '../shared';
...
async function handleCronImport(request: Request) {
  ...
  const autoImportState = await getLookerLeadsAutoImportState();
  ...
  if (!autoImportState.enabled) {
    ...
    if (supabaseUrl && serviceRoleKey) {
      try {
        ...
        await supabase.from('looker_leads_import_runs').insert({
          triggered_by: 'cron',
          status: 'skipped',
